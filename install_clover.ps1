$clover_database_user = "clover"
$clover_database_db   = "clover_db"
...
function setUserWritablePermissions
{
    ...
    $user = "Users"
    $Rights = "Write, Read, ReadAndExecute"
...
./setCloverServerProperties.ps1 -Database_engine $rds_engine -Database_url $clover_database_url -Database_db $clover_database_db -Database_user $clover_database_user -Database_pass "$clover_database_password"
...
$BaseDir = $tomcatDir
$NameToFind = "Clover"
$cloverTomcatZipName = $cloverTomcatZip.Replace(".zip","")
#$MyVariable = Get-ChildItem $BaseDir -Recurse | Where-Object { $_.PSIsContainer -and $_.Name.StartsWith($NameToFind)}
Write-Output "Source Directory: ${tomcatDir}\${cloverTomcatZipName}"
Write-Output "Target Directory: ${tomcatDirectory}"

New-Item $tomcatDirectory\webapps\clover -ItemType directory
Copy-Item -Path ${tomcatDir}\${cloverTomcatZipName}\webapps\clover.war -Destination $tomcatDirectory\webapps\clover\clover.war
...
Copy-Item -Path ${tomcatDir}\${cloverTomcatZipName}\webapps\profiler.war -Destination $tomcatDirectory\webapps\profiler\profiler.war
...
elseif ($rds_engine -eq "postgres") {
Copy-Item -Path $clover_assets\postgresql-42.3.1.jar -Destination $tomcatDirectory\webapps\clover\WEB-INF\lib\postgresql-42.3.1.jar
Write-Output "Move Files (postgres) - Time taken: $((Get-Date).Subtract($start_time).Seconds) second(s)"
Write-Output "Postgres lib files added"
}
