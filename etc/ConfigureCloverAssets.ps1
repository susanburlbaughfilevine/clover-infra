$config = Import-PowershellDataFile c:\clover-assets\clover-assets-manifest.psd1

# Delete old JDK and Tomcat directories. If we need to revert due to an issue, deploy an older release
Remove-Item C:\jdk -Recurse -Force
Remove-Item C:\tomcat -Recurse -Force

# Extract/Install JDK and Apache Tomcat
$tomcatDirectory = New-Item -Type Directory -Path c:\tomcat
$jdkDirectory = New-Item -Type Directory -Path c:\jdk
...
Expand-Archive C:\clover-assets\$($config["tomcat"].PackageName) -Destination $tomcatDirectory.FullName
Expand-Archive C:\clover-assets\$($config["jdk"].PackageName) -Destination $jdkDirectory.FullName
...
Copy-Item -Path C:\clover-assets\cloverServer.properties -Destination "$tomcatPath\conf\"
$setEnvScript = (Get-Content -Path C:\clover-assets\setenv.bat).Replace("##tomcatConfDir##","$tomcatPath\conf\cloverServer.properties")
